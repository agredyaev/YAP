...
def calculate_user_interests(date: str, depth: str, events_base_path: str, output_base_path: str, spark) -> None:
    ...
    paths = input_paths(date, depth, events_base_path)
    ...
    print('Stage 3: load all messages')
    ...
    print('Stage 4: load top reactions')
    ...
    print('Stage 5: load top likes and dislikes')
    ...
    print('Stage 6: join all')

    result.write.mode("overwrite").parquet(f"{output_base_path}/user_interests_d{depth}")
    
    print(f"Stage 7: Save file to {output_base_path}/user_interests_d{depth}")
